...
def create_test_data(apps, schema_editor):
    ...
    Kind.objects.create(name="Siamese")

    Kind.objects.create(name="Asian")

    Cat.objects.create(
        color="Gray", age=3, description="Test cat1", kind_id=1, user_id=1
    )

    Cat.objects.create(
        color="Black", age=24, description="Test cat2", kind_id=2, user_id=2
    )
